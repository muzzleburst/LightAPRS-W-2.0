...
const std::string PicoBalloon_::getComment() const
{
    mutex_enter_blocking(&commentMutex);
...
void PicoBalloon_::setComment(const std::string& message)
{
    mutex_enter_blocking(&commentMutex);
    strcpy(comment, message.c_str());
    mutex_exit(&commentMutex);
}

const std::string PicoBalloon_::getStatus() const
{
    mutex_enter_blocking(&StatusMessageMutex);
...
void PicoBalloon_::setStatus(const std::string& message)
{
    mutex_enter_blocking(&StatusMessageMutex);
    strcpy(StatusMessage, message.c_str());
    mutex_exit(&StatusMessageMutex);
}

void PicoBalloon_::begin(const std::string& callSign, uint8_t callNumber)
{
  for (int i=0; i<7; i++)
  {
    CallSign[i] = callSign[i];
    hf_call[i] = callSign[i];
  }
...
{
    static PicoBalloon_ instance;
    return instance;
}

PicoBalloon_ &PicoBalloon = PicoBalloon_::getInstance();
